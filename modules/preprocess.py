...
def train_test_split(df,df_labels, cfg):
    if cfg.cv:
        skf = StratifiedKFold(n_splits=cfg.n_splits, shuffle=True, random_state=1023)
        categorical_labels = df_labels.idxmax(axis=1)
        train_index, valid_index = next(skf.split(df, categorical_labels))
        df_train = df.loc[train_index].reset_index(drop=True)
        df_valid = df.loc[valid_index].reset_index(drop=True)
        df_labels_train = df_labels.loc[train_index].reset_index(drop=True)
        df_labels_valid = df_labels.loc[valid_index].reset_index(drop=True)    
    else:
        df_train = df
...
def preprocess(cfg):
    ...
    if cfg.debug:
        df = df[df[cfg.primary_label_col].isin(cfg.bird_cols_train)].reset_index(drop=True)
        df = df.groupby(cfg.primary_label_col).apply(lambda x: x.sample(n=min(5,len(x)))).reset_index(drop=True)
    ...
    df['path'] = df['filepath'].apply(lambda x: os.path.join(cfg.train_dir, x))
