...
def roc_auc(solution, submission):
...
class BirdClefModelBase(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.valid_step_step_outputs.append({"val_loss": val_loss, "logits": y_pred, "targets": target})
    ...
    def on_validation_epoch_end(self):
        outputs = self.valid_step_step_outputs
        if len(outputs):
            ...
            if self.current_epoch > -1:
                avg_score = roc_auc(val_df, pred_df)
                self.log(
                    "val_loss",
                    avg_loss,
                    on_step=False,
                ...
                print(
                    f"epoch {self.current_epoch} validation ROC-AUC score {avg_score}"
                )
            else:
                self.log(
                    "val_loss",
